...
class PayCalculator {
    ...
    fun calculatePay(entry: TimeEntry, settings: Settings): TimeEntry {
        ...
        val obPay = calculateOBPay(entry, settings, workHours)
        val totalPay = basePay + obPay
    ...
    private fun calculateOBPay(entry: TimeEntry, settings: Settings, workHours: Double): Double {
        if (entry.startTime == null || entry.endTime == null || workHours <= 0) return 0.0
        
        val dayOfWeek = entry.date.dayOfWeek
        val obRates = settings.obRates
        var obPay = 0.0
        
        when (obRates.workplaceType) {
            WorkplaceType.BUTIK -> {
                obPay = calculateButikOBPay(entry, settings, workHours, dayOfWeek)
            }
            WorkplaceType.LAGER -> {
                obPay = calculateLagerOBPay(entry, settings, workHours, dayOfWeek)
            }
        }
    ...
    private fun calculateButikOBPay(entry: TimeEntry, settings: Settings, workHours: Double, dayOfWeek: DayOfWeek): Double {
        val obRates = settings.obRates
        var obPay = 0.0
        
        // Red day gets 100% bonus for all hours
        if (entry.isRedDay) {
            obPay += workHours * settings.basePay * obRates.butikRedDayAllDay
        }
        // Sunday gets 100% bonus for all hours
        else if (dayOfWeek == DayOfWeek.SUNDAY) {
            obPay += workHours * settings.basePay * obRates.butikSundayAllDay
        }
        // Saturday gets 100% bonus after 12:00
            ...
            if (entry.endTime!!.isAfter(saturdayOBTime)) {
                ...
                obPay += obHours * settings.basePay * obRates.butikSaturdayAfter1200
            }
        }
        // Weekdays (Monday-Friday)
            ...
            if (entry.endTime!!.isAfter(weekday1815)) {
                ...
                obPay += obHours1815 * settings.basePay * obRates.butikWeekday1815to2000
            }
            ...
            if (entry.endTime.isAfter(weekday2000)) {
                ...
                obPay += obHours2000 * settings.basePay * obRates.butikWeekdayAfter2000
            }
        }
    ...
    private fun calculateLagerOBPay(entry: TimeEntry, settings: Settings, workHours: Double, dayOfWeek: DayOfWeek): Double {
        val obRates = settings.obRates
        var obPay = 0.0
        
        // Red day gets 100% bonus for all hours
        if (entry.isRedDay) {
            obPay += workHours * settings.basePay * obRates.lagerRedDayAllDay
        }
        // Sunday gets 100% bonus for all hours
        else if (dayOfWeek == DayOfWeek.SUNDAY) {
            obPay += workHours * settings.basePay * obRates.lagerSundayAllDay
        }
        // Monday special night rate 00:00-06:00
        else if (dayOfWeek == DayOfWeek.MONDAY) {
            val mondayNightStart = LocalTime.of(0, 0)
            val mondayNightEnd = LocalTime.of(6, 0)
            obPay += calculateOBForTimeRange(entry, settings, mondayNightStart, mondayNightEnd, obRates.lagerMondayNight0000to0600)
        }
        // Saturday has different rates
            ...
            obPay += calculateOBForTimeRange(entry, settings, satNight1Start, satNight1End, obRates.lagerSaturdayNight0000to0600)
            obPay += calculateOBForTimeRange(entry, settings, satDayStart, satDayEnd, obRates.lagerSaturdayDay0600to2300)
            obPay += calculateOBForTimeRange(entry, settings, satNight2Start, satNight2End, obRates.lagerSaturdayNight2300to2400)
        }
        // Weekdays (Tuesday-Friday for lager)
            ...
            obPay += calculateOBForTimeRange(entry, settings, morningStart, morningEnd, obRates.lagerWeekdayMorning0600to0700)
            obPay += calculateOBForTimeRange(entry, settings, eveningStart, eveningEnd, obRates.lagerWeekdayEvening1800to2300)
            obPay += calculateOBForTimeRange(entry, settings, nightStart, nightEnd, obRates.lagerWeekdayNight2300to0600)
        }
